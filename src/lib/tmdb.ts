...
const DEFAULT_TMDB_API_KEY = "802b2c4b88ea1183e50e6b285a27696e";
...
async function tmdb<T>(path: string, params: Record<string, string | number | undefined> = {}): Promise<T> {
  ...
  if (apiKey && !process.env.TMDB_READ_ACCESS_TOKEN) query.set("api_key", apiKey);
...
export function normalizeSearchResult(raw: z.infer<typeof resultSchema>): Title {
  ...
  return {
    ...
    posterUrl: image(raw.poster_path),
    backdropUrl: image(raw.backdrop_path),
    releaseDate,
...
export async function searchTitles(query: string) {
  ...
  return data.results.flatMap((item) => {
    ...
    return parsed.success && parsed.data.media_type !== "person" ? [normalizeSearchResult(parsed.data)] : [];
  });
}
...
export async function getTitle(id: number, type: TitleType, region = process.env.TMDB_DEFAULT_REGION ?? "US"): Promise<Title> {
  ...
  const parsed = resultSchema
    .extend({
      runtime: z.number().nullable().optional(),
      episode_run_time: z.array(z.number()).optional(),
      genres: z.array(z.object({ name: z.string() })).default([]),
      seasons: z.array(z.object({ season_number: z.number(), name: z.string(), episode_count: z.number() })).default([]),
          ...
          results: z
            .record(
              ...
              z
                .object({
                  flatrate: z
                    .array(z.object({ provider_id: z.number(), provider_name: z.string(), logo_path: z.string().nullable().optional() }))
                    .optional(),
  ...
  return {
    ...
    posterUrl: image(parsed.poster_path),
    backdropUrl: image(parsed.backdrop_path),
    releaseDate,
    year: releaseDate?.slice(0, 4) ?? null,
    runtimeMinutes: parsed.runtime ?? parsed.episode_run_time?.[0] ?? null,
    genres: parsed.genres.map((genre) => genre.name),
    rating: parsed.vote_average ?? null,
    voteCount: parsed.vote_count ?? 0,
    imdbId: parsed.external_ids?.imdb_id ?? null,
    seasons: parsed.seasons.map((season) => ({ seasonNumber: season.season_number, name: season.name, episodeCount: season.episode_count })),
    cast: (parsed.credits?.cast ?? []).slice(0, 8).map((person) => ({ id: person.id, name: person.name, character: person.character, profileUrl: image(person.profile_path) })),
    watchProviders: providers.map((provider) => ({ providerId: provider.provider_id, providerName: provider.provider_name, logoUrl: image(provider.logo_path), link: parsed["watch/providers"]?.results?.[region]?.link ?? null })),
  };
}
